...
test("search, status and priority combine without changing source data", () => {
  ...
  assert.equal(
    selectTasks(tasks, { search: "DEMO", status: "Active", priority: "High" })
      .length,
    0,
  );
...
test("statistics count underlying tasks, including empty and completed lists", () => {
  assert.deepEqual(taskStatistics(tasks), {
    total: 3,
    completed: 1,
    pending: 2,
  });
  ...
  assert.deepEqual(
    taskStatistics(tasks.map((t) => ({ ...t, completed: true }))),
...
test("all supported sorts work on filtered results and preserve source order", () => {
  ...
  assert.deepEqual(
    selectTasks(tasks, { sort: "oldest" }).map((t) => t.id),
    ["1", "2", "3"],
  );
  assert.deepEqual(
    selectTasks(tasks, { sort: "priority" }).map((t) => t.id),
    ["2", "3", "1"],
  );
  assert.deepEqual(
    selectTasks(tasks, { sort: "title" }).map((t) => t.id),
    ["3", "1", "2"],
  );
  assert.deepEqual(
    selectTasks(tasks, { status: "Active", sort: "priority" }).map((t) => t.id),
    ["3", "1"],
  );
  assert.deepEqual(
    tasks.map((t) => t.id),
...
test("literal markup and accented task titles are ordinary searchable text", () => {
  ...
  assert.equal(
    selectTasks([{ title: "Café break", priority: "Low", completed: false }], {
      search: "CAFÉ",
    }).length,
    1,
  );
...
test("focus queue puts user pins first, then priority and age; excludes completed tasks", async () => {
  ...
  assert.deepEqual(
    getFocusQueue(sample).map((t) => t.id),
    ["b", "c", "a"],
  );
  assert.deepEqual(
    sample.map((t) => t.id),
