...
export function selectTasks(tasks, {status = 'All', priority = 'All', search = '', sort = 'newest'} = {}) {
  const query = search.trim().toLocaleLowerCase();
  const rank = {High:0, Medium:1, Low:2};
  return tasks.filter(task => (status === 'All' || task.completed === (status === 'Completed')) && (priority === 'All' || task.priority === priority) && task.title.toLocaleLowerCase().includes(query)).sort((a,b) => {
    if(sort === 'priority') return rank[a.priority] - rank[b.priority] || (b.createdAt || 0) - (a.createdAt || 0);
...
export function getFocusQueue(tasks) {
  ...
  const rank = {High:0,Medium:1,Low:2};
  return active.sort((a,b)=>Number(!!b.focused)-Number(!!a.focused) || rank[a.priority]-rank[b.priority]);
}
