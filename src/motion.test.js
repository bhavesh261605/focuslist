...
const motion=readFileSync(new URL('./motion.css',import.meta.url),'utf8').replace(/\/\*[\s\S]*?\*\//g,'');
test('motion tokens stay within 150–300ms, with no linear easing or delays',()=>{
  const durations=[...motion.matchAll(/:(\d+)ms/g)].map(x=>Number(x[1]));
...
test('transitions only interpolate transform and opacity; legacy motion is retired',()=>{
  for(const match of motion.matchAll(/transition:([^;}]+)/g)){
    if(match[1].startsWith('none'))continue;
    const names=match[1].split(/,(?![^()]*\))/).map(s=>s.trim().split(' ')[0]);
    assert.ok(names.every(n=>['transform','opacity'].includes(n)),names.join(','));
  }
